...
% ========== COMPUTED QUANTITIES ========= %
W_ave = 0.5*(W_rest + W_exer);
W = @(v)myTrans(v,ts,W_rest,0.5*(W_exer-W_rest)*sin(pi*(v-ts-0.5))+W_ave);
apesk_ave = 0.5*(A_pesk_rest + A_pesk_exer);
A_pesk = @(v)myTrans(v,ts,A_pesk_rest,(apesk_ave+0.5*(A_pesk_exer - A_pesk_rest)*sin(pi*(v-ts-0.5))));
rp_ave = 0.5*(1.5446+0.3);
R_p = @(v)myTrans(v,ts,R_p_rest,(rp_ave+0.5*(R_p_exer - A_pesk_rest)*sin(pi*(v-ts-0.5))));
