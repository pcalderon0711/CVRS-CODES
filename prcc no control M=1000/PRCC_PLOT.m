...
for i=1:k
    ...
    figure,plot((eval(c4)),(eval(c5)),'.','MarkerSize',30),title(a),...
            legend(PRCC_var{i}),xlabel(PRCC_var{i},'FontSize',20),ylabel('res,P_{as}','FontSize',20);%eval(c6);
    set(gca,'fontsize',17);
    cd 'scatter'
    savefig(sprintf('param_%d_time_%d',i,s));
    print(sprintf('param_%d_time_%d',i,s),'-dpng'); 
    cd ..
end
