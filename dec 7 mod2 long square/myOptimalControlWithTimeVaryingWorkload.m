...
x_nom(1,1:N+1) = x1(1)*ones(1,N+1);
for i=1:N+1
    j = i-1;
    if (50<=j)&&(j<90) || ((110<=j)&&(j<=150))
        x_nom(1,i) = xz(1);
    end
end
...
W = @(t)myTrans(t,ts,0,myLongSquare(t-ts,0,75));
A = @(t)myTrans(t,ts,177.682,myLongSquare(t-ts,177.682,270)); %A_pesk function
R_p = @(t)myTrans(t,ts,1.5446,myLongSquare(t-ts,1.5446,0.3));
w = 'longsquare';
% 
