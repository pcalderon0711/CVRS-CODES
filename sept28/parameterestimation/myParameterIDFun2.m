...
restHR = HR15(1:1+(length(HR15)-1)/3);
restMAP = MAP15(1:1+(length(HR15)-1)/3);
exerHR = HR15(2+(length(HR15)-1)/3:end);
...
mkdir(code);
cd(code);
save(sprintf('%s_param_estimation.mat',code),'t','T','N','ts','P0','paramsetID','P','resnorm','residual','exitflag','output','params_rest','params_exer');
cd ../..
