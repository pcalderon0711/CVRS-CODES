function dy = mySensitivityModel(t,Y,params_rest,params_exer,t1)
...
% columns of z represent parameters, rows represent states
% (i,j)th entry of z is the sensitivity of ith state wrt jth param
z = reshape(Y(13:end),12,36);
...
% Use automatic differentiation to get jacobians dFdx and dFdu.
x = Y(1:12);
xAD = myAD(x);
xoutAD = myModel(xAD,params);
dFdx = getderivs(xoutAD);
pAD = myAD(params);
poutAD = myModel(x,pAD);
dFdu = getderivs(poutAD);

% Dynamics of sensitivities of states with respect to parameters
dz = zeros(12,36);
for i=1:12
    for j=1:36
        dz(i,j) = dFdu(i,j);
        for k=1:12
            dz(i,j) = dz(i,j) + dFdx(i,k)*z(k,j);
        end
    end
end

dy(12+1:12+12*36) = reshape(dz,12*36,1);