...
k = 33 + 1; % # of input factors (parameters varied) + dummy parameter
...
Y(NS,length(time_points),14,length(pmin),NR)=0;  % pre-allocation
...
for i=1:k % i=# of replications (or blocks)
    ...
    for L=1:NR
        ...
        % uniform to general.
        X(:,:,i,L) = parameterdist(X(:,:,i,L),pmax,pmin,pmean,pstd,NS,'norm'); %%this is what assigns 'our' values rather than 0:1 dist
...
%             [t,y]=ode15s(@(t,y)f(t,y,X(:,:,i,L),run_num),tspan,y0,[]); 
            ...
            y(1,:) = myEquilibriumSolver(X(:,:,i,L),run_num,78.5,40,0);
            for index=1:length(tspan)-1
                f = @ODE_efast;
                if index == 1
                    prev = y(1,:);
                end
                implicit = @(next)(next - prev - h*f(tspan(index+1),next,u,X(:,:,i,L),run_num,tspan,ts));
                y(index+1,:) = fsolve(implicit, prev, opts);
                prev = y(index+1,:);
            end
