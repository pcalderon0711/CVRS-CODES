...
params = myLoader('parameters_exer.txt','p'); % load exercise parameters
x1 = myEquilibriumSolver(params_rest,78.5,40); % calculate rest equilibrium
xz = myEquilibriumSolver(params,107,40); % calculate exercise equilibrium

N = 100; % set number of time points
alpha = 0; % set penalization parameter on O2
beta = 0; % set penalization parameter on CO2

T = 10; % set terminal time
h = T/N; % calculate length of each subinterval
t = linspace(0,T,N+1); % partition time interval [0,T] into N+1 equal subintervals
...
%% optimization stuff
f = @(t,x)(myModelWithControlAndTimeVaryingWorkload(t,x,params,W,A,R_p)); % construct vector of the dynamics of states
jac = @(t,x)(myDerivativesTimeVarying(t,x,params,A,R_p)); % construct matrix of the jacobian of f
J = @(z)myCostVectorWithPenalizationAndTimeVaryingWorkload(z, f, jac, B, R, Q, h, x1, x_nom, params, alpha,beta, W,A); % construct cost vector
