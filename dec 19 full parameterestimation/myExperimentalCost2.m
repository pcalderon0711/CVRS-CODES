function J = myExperimentalCost2(P,a,pas_data,h_data,params_rest,params_exer,averageH_rest,averageH_exer,N,t1,t2,t3,t4,t5,t6,h,paramset_idx)
...
x_nom(1,1:1+N*(5/60)) = x1_1(1);  % set nominal value of Pas, first 12 minutes
x_nom(1,1+N*(5/60)+1:1+N*(5/60)+N*(15/60)) = xz_1(1);
x_nom(1,1+N*(20/60)+1:1+N*(32/60)) = x1_2(1);
x_nom(1,1+N*(32/60)+1:1+N*(47/60)) = xz_2(1);
x_nom(1,1+N*(47/60)+1:1+N*(50/60)) = x1_3(1);
x_nom(1,1+N*(50/60)+1:1+N*(55/60)) = xz_3(1);
x_nom(1,1+N*(55/60)+1:1+N) = x1_4(1);

%% set workload, apesk and Rp

W = @(t)myTrans(t,t1,t2,t3,t4,t5,t6,0,75);
...
x = [x1_1,z(1:N,:)']; % each column is one time step, vector of states
...
rest_indices = [1:1+N*(5/60), 2+N*(20/60):1+N*(32/60), 2+N*(47/60):1+N*(50/60), 2+N*(55/60):1+N];
exer_indices = [2+N*(5/60):1+N*(20/60), 2+N*(32/60):1+N*(47/60), 2+N*(50/60):1+N*(55/60)];
    
for j=1:length(rest_indices) % in total, there are N+1 data points, but first data point is at t=0, so we start at n=2
    i = rest_indices(j);
    K(i) = a1rest*(x(1,i)-pas_data(i))^2;
end
for j=1:length(exer_indices)
    i = exer_indices(j);
